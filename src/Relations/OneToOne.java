...
public class OneToOne<T,FK> {
	...
	public void change(Set<FK> setForUpdate,DbConnection<?, ?> conn, T object, Object id) {
		...
		while(it.hasNext()){
			...
			Object Id = conn.executeInsert(queryUpdate, null);
		...
		try {
			FK object2 = (FK)setForUpdate.iterator().next();
			Method methodSet = object.getClass().getMethod("set" + object2.getClass().getName(),setForUpdate.getClass());
