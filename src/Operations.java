...
public abstract class Operations<T,PK, FK>{
	...
	public Operations(DbConnection<?, ?> db, Class<T> classToCreate, Set<FK> set){
		...
		setTable(classToCreate);		//setam numele tabelului
		setColumns(classToCreate);		//setam numele coloanelor
	}
	
	public void setColumns(Class<?> classToCreate){
		columns.clear();
	...
	public void setTable(Class<?> classToCreate){
		...
		if(ann instanceof Table){
			tableName = ((Table) ann).name().toString();
		}
	}
	...
	public synchronized Boolean createTable(){
		...
		while(itCol.hasNext()){
			String val = itCol.next().toString();
			sb.append(val + " ");
	...
	protected void setFK(){
		for(Field field: classToCreate.getDeclaredFields()){
			...
			if(annotation != null){
				...
				if(col.column().compareTo("") != 0){
					String query = "ALTER TABLE " + tableName + " ADD CONSTRAINT fk_" + new Random().nextInt(100) + " FOREIGN KEY ("+field.getName()+") REFERENCES "+col.table() + " ("+col.column()+")";
	...
	protected Boolean executeQueryInsert(String[] fieldsQuery, String[] valuesQuery,Method setId, T object,Boolean makeUpdateOnForeign,Set<FK> setForUpdate){
		...
		try {
				id = (Long) conn.executeInsert(queryInsert,new String[1][1]);
		...
		for(Field field : fields){
			...
			if(annotation != null){
					...
					if(select.selected()){
						Method method;
						try {
							method = classToCreate.getMethod("get" + field.getName().toString().substring(0,1).toUpperCase()+field.getName().toString().substring(1, field.getName().toString().length()), new Class[] {});
	...
	protected Method getMethod(String methodName,Class<T> classToCreate, Class<?> annotationClass){
